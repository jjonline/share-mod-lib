...
// New 创建钉钉客户端
//   - webhook  	钉钉Webhook的URL
//   - secret   	钉钉secret，钉钉机器人设置时 启用加签获得以 SEC 开头的秘钥令牌
//   - client   	自定义 *http.Client 可自主控制http请求客户端，给 nil 不则使用默认
//   - switchFunc   开关，true则真实发送 false则不真实发送/不用更改注释调用代码仅初始化时设置该值即可关闭真实发送逻辑
func New(webhook, secret string, client *http.Client, switchFunc func() bool) *Ding {
	return &Ding{
		config: &config{
			webhook: webhook,
			secret:  secret,
...
// Once 配置单次<once>使用其他钉钉机器人
//   - webhook    钉钉webhook
//   - secret     钉钉webhook对应的秘钥
func (d *Ding) Once(webhook, secret string) *Ding {
	d.once = &config{
		webhook: webhook,
		secret:  secret,
	}
...
func (d *Ding) removeOnce() {
	d.once = nil
}

func (d *Ding) sign(t int64) string {
	var secret = d.config.secret
	if d.once != nil {
		secret = d.once.secret
	}
...
func (d *Ding) send(message interface{}) error {
	defer d.removeOnce() // remove once config anyway
	...
	params := url.Values{}
	if cnf.secret != "" { // 如果设置密钥,则签名
		t := time.Now().Unix() * 1000
		params.Set("timestamp", strconv.FormatInt(t, 10))
		params.Set("sign", d.sign(t))
	}

	res, err := d.client.PostJSON(context.TODO(), guzzle.ToQueryURL(cnf.webhook, params), message, nil)
	if err != nil {
		return err
	}
