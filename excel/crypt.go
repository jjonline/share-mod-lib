...
func Decrypt(raw []byte, opt *excelize.Options) (packageBuf []byte, err error) {
...
func Encrypt(raw []byte, opt *excelize.Options) (packageBuf []byte, err error) {
...
func encryptionMechanism(buffer []byte) (mechanism string, err error) {
	if len(buffer) < 4 {
		err = excelize.ErrUnknownEncryptMechanism
		return
	}
...
func standardDecrypt(encryptionInfoBuf, encryptedPackageBuf []byte, opt *excelize.Options) ([]byte, error) {
...
func standardConvertPasswdToKey(header StandardEncryptionHeader, verifier StandardEncryptionVerifier, opt *excelize.Options) ([]byte, error) {
...
func (e *encryption) standardKeyEncryption(password string) ([]byte, error) {
	if len(password) == 0 || len(password) > excelize.MaxFieldLength {
...
// SHA384 and SHA512.
func agileDecrypt(encryptionInfoBuf, encryptedPackageBuf []byte, opt *excelize.Options) (packageBuf []byte, err error) {
...
func genISOPasswdHash(passwd, hashAlgorithm, salt string, spinCount int) (hashValue, saltValue string, err error) {
	if len(passwd) < 1 || len(passwd) > excelize.MaxFieldLength {
