...
class KeywordExtractor {
 public:
  ...
  void Extract(const string& sentence, vector<Word>& keywords, size_t topN) const {
    ...
    for (map<string, Word>::iterator itr = wordmap.begin(); itr != wordmap.end(); ++itr) {
      ...
      if (cit != idfMap_.end()) {
        ...
        if(userCustomWordCount_>0){
            itr->second.weight *= (idfAverage_+ (userCustomIdfSum_/userCustomWordCount_))/2;
        } else {
  ...
  void SetWordsIdf(string word,double idf) {
      AddWordIdf(word,idf);
  }

 private:
 void AddWordIdf(string word,double idf) {
     userCustomWordCount_++;
     idfMap_[word] = idf;
     userCustomIdfSum_ += idf;
     assert(userCustomWordCount_ > 0);
   }
  void LoadIdfDict(const string& idfPath) {
    userCustomIdfSum_ = 0.0;
    userCustomWordCount_= 0;
  ...
  double userCustomIdfSum_;
  size_t userCustomWordCount_;
}; // class KeywordExtractor
